...
from Dataset import Sketchy_data
import random
os.environ['RANK']='0'
...
if __name__ == '__main__':
    ...
    # dataset
    val_dataset = Sketchy_data(args=opt, mode='test')
    ...
    ckp_fus = torch.load('models/MaskV2/fusionnet.pth', map_location='cpu')
    ...
    if rank==0:
        for index, data in enumerate(val_dataloader):
            ...
            # cv2.imwrite(os.path.join(experiments_root, 'visualization', str(index), 'mask.png'), tensor2img(data['deformed_mask']))
            # continue
            with torch.no_grad():
                if opt.dpm_solver:
                ...
                z1 = model.module.encode_first_stage((data['pixel_value']*2-1.).to(torch.float32).cuda(non_blocking=True))
                ...
                #cv2.imwrite(os.path.join(experiments_root, 'visualization', str(index), 'edge2.png'), tensor2img(1-edge[1:]))
                cv2.imwrite(os.path.join(experiments_root, 'visualization', str(index), 'ori_img1.png'), tensor2img(data['pixel_value'][0:1]))
                #cv2.imwrite(os.path.join(experiments_root, 'visualization', str(index), 'ori_img2.png'), tensor2img(data['pixel_value'][1:]))
                cv2.imwrite(os.path.join(experiments_root, 'visualization', str(index), 'gt_img1.png'), tensor2img(data['deformed_value'][0:1]))
                #cv2.imwrite(os.path.join(experiments_root, 'visualization', str(index), 'gt_img2.png'), tensor2img(data['deformed_value'][1:]))
                ...
                x_samples_ddim = x_samples_ddim*gen_mask + (data['pixel_value'].to(torch.float32).cuda(non_blocking=True))*(1-gen_mask)
