...
from path_utils import DEFAULT_CHECKPOINT_ROOT, DEFAULT_OUTPUT_ROOT, apply_checkpoint_config, resolve_common_paths
...
def load_resume_state(opt):
    ...
    if opt.auto_resume:
        state_path = osp.join(opt.output_dir, opt.name, 'training_states')
...
parser.add_argument(
        ...
        help="root directory for logs, checkpoints, and visualizations",
)
parser.add_argument(
        "--data_path",
        type=str,
...
opt = resolve_common_paths(opt, dataset_parts=["sketchy"], legacy_dataset="./Dataset/sketchy")
...
if __name__ == '__main__':
    ...
    # dataset
    train_dataset = Sketchy_data(args=opt, mode='train')
    ...
    experiments_root = osp.join(opt.output_dir, opt.name)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for _, data in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                edge1 = edge1>0.5
                edge2 = edge2>0.5
                rough_mask = data['deformed_mask'].to(torch.float32).cuda(non_blocking=True)
                edge = (edge2.float()-edge1.float()).clip(min=0)*rough_mask
            ...
            # save checkpoint
            rank, _ = get_dist_info()
            if ((rank==0) and ((current_iter+1)%config['training']['save_freq'] == 0)) or current_iter==1:
                print("saving model")
